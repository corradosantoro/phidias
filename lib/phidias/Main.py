...
class SHELL:
    ...
    def run(self):
        if sys.implementation.name != "micropython":
            c = WordCompleter(["help", "assert", "+", "retract", "-", "agents", "agent",
        ...
        while True:
            if sys.version_info[0] == 2:
                s = prompt("pShell: {0} > ".format(self.__current_agent), completer=c).strip()
            else:
                s = prompt("eShell: {0} > ".format(self.__current_agent), completer=c).strip()
            if s == "":
                continue
            if s[0] == '+':
                s = "assert " + s[1:]
